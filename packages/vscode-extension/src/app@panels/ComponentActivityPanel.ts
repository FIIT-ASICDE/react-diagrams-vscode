...
import { CancellationTokenSource, Disposable, Range, TextDocument, TextEditor, Webview, WebviewPanel, window, Uri, ViewColumn, workspace } from "vscode";
...
export class ComponentActivityPanel {
    ...
    private async publishActiveEditorCode() {
        ...
        this.postMessage({
            ...
            data: {
                nodes: parsedComponent.nodes,
                edges: parsedComponent.edges,
                sourceFile: bestDocument.uri.fsPath,
            },
        });
    }

    private async generateAndEnrichSkeletonFromDiagram(nodes: Node[], edges: Edge[], activeFilePath?: string) {
        if (!nodes.length) {
        ...
        const generatedEditor = await window.showTextDocument(generatedDocument, ViewColumn.Beside, true);
    ...
    private unwrapCodeFence(text: string): string {
        ...
        const fenceMatch = trimmed.match(/^```(?:typescript|ts|tsx|javascript|js)?\s*\n([\s\S]*?)\n```\s*$/i);
        if (fenceMatch?.[1]) {
            return fenceMatch[1].trim();
        }

        const singleQuoteFenceMatch = trimmed.match(/^'''(?:typescript|ts|tsx|javascript|js)?\s*\n([\s\S]*?)\n'''\s*$/i);
            ...
            case "diagram/generateSkeleton": {
                ...
                void this.generateAndEnrichSkeletonFromDiagram(
                    ...
                    typeof payload.sourceFile === "string"
                        ? payload.sourceFile
                        : this.getBestEditorForCode()?.document.uri.fsPath,
                );
                return;
            }
