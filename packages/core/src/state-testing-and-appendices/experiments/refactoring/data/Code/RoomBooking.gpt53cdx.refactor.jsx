...
export default function RoomBooking() {
  ...
  const isBusy = status === "loading" || status === "submitting" || status === "cancelling";
  const isSlotBlocked = slot => isBusy || takenSlots.includes(slot);
  ...
  useEffect(() => {
    ...
    async function loadSlots() {
      ...
      try {
        const result = await fetchExistingBookings(selectedRoom);
        if (stale) return;
  ...
  async function handleBook() {
    ...
    try {
      const bookingResult = await submitBooking(selectedRoom, selectedSlot, note);
    ...
    <div style={{ maxWidth: 580, margin: "0 auto", padding: "32px 18px", fontFamily: "Georgia, serif", color: "#1a1a1a" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <select
          ...
          disabled={isBusy}
          style={{ width: "100%", padding: "10px 12px", fontSize: 14, borderRadius: 8, border: "1.5px solid #ddd", background: "#fafafa", cursor: isBusy ? "not-allowed" : "pointer" }}
        >
          {ROOMS.map(r => (
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {SLOTS.map(slot => {
            ...
            if (isTaken) {
              ...
              border = "#1a1a1a";
              color = "#fff";
            } else if (status === "confirmed" && !isSelected) {
              cursor = "default";
            }
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        <button
          onClick={handleBook}
          disabled={isBusy || status === "confirmed" || status === "error"}
          style={{ flex: 1, padding: "11px 0", borderRadius: 8, border: "none", background: (isBusy || status === "confirmed" || status === "error") ? "#e5e5e5" : "#1a1a1a", color: (isBusy || status === "confirmed" || status === "error") ? "#999" : "#fff", fontWeight: 700, fontSize: 14, cursor: (isBusy || status === "confirmed" || status === "error") ? "not-allowed" : "pointer" }}
        >
