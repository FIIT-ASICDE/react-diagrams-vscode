...
function markSelectedAsPaid(invoices: Invoice[], selectedId: number) {
  let updatedInvoices = invoices;

  for (let i = 0; i < updatedInvoices.length; i++) {
    if (updatedInvoices[i].id === selectedId) {
      updatedInvoices = [
...
export default function InvoiceReview() {
  ...
  async function handleSend(canSend: boolean) {
    ...
    try {
      ...
      setInvoices(markSelectedAsPaid(invoices, selected.id));
      setReviewState("sent");
