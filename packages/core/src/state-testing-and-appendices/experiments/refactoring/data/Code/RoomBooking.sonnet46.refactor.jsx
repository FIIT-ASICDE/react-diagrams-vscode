...
export default function RoomBooking() {
  ...
  useEffect(() => {
    ...
    async function loadSlots() {
      ...
      try {
        ...
        if (!stale) { setTakenSlots(result); setStatus("ready"); }
      } catch {
        if (!stale) { setMessage("Could not load availability. Try again."); setStatus("error"); }
      }
    }
  ...
  async function handleBook() {
    ...
    if (!selectedSlot)                          { setMessage("Please select a time slot.");                    setStatus("ready"); return; }
    if (takenSlots.includes(selectedSlot))      { setMessage("That slot was just taken. Please pick another."); setStatus("ready"); return; }
    if (note.length > noteCharLimit)            { setMessage("Note is too long.");                             setStatus("ready"); return; }
    if (submitCount.current >= 3)               { setMessage("Too many booking attempts. Please wait.");        setStatus("error"); return; }
  ...
  function handleSlotSelect(slot) {
    if (status === "loading" || status === "submitting" || status === "confirmed" || takenSlots.includes(slot)) return;
    setSelectedSlot(slot);
    if (status === "error") { setStatus("ready"); setMessage(""); }
  }
