...
export default function InvoiceReview() {
  ...
  async function handleSend(canSend: boolean) {
    ...
    try {
      ...
      setInvoices(invoices.map(inv => inv.id === selected.id ? { ...inv, paid: true } : inv));
      setReviewState("sent");
    ...
    switch (action) {
      case "archive-paid":
        if (invoices.every(inv => inv.paid))
          setInvoices(prev => prev.filter(inv => !inv.paid));
        setReviewState("archived");
        break;

      case "flag-large": {
        const hasLarge = invoices.some(inv => inv.amount > 3000);
        setInvoices(invoices.map(inv => inv.amount > 3000 ? { ...inv, flagged: true } : inv));
        if (hasLarge) setReviewState("flagged");
        break;
      }
