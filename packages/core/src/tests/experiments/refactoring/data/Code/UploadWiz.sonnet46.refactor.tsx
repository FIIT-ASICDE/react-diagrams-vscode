...
const FORBIDDEN_NAME_PARTS = ["virus", "tmp", "backup"];
...
function validateFile(file: File): string | null {
  ...
  if (file.name.split(".").length - 1 > 2) {
    return "The file name contains too many extensions.";
  }

  if (file.size === 0) return "The file is empty.";
  if (file.size > 5_000_000) return "The file is too large.";
...
export default function UploadWizard() {
  ...
  async function startUpload() {
    ...
    if (retryCount.current > 2) {
      failWith("Upload retry limit reached.");
      return;
    }
    ...
    try {
      for (let part = 1; part <= 5; part++) {
        ...
        if (lastFileName.current.endsWith(".exe")) {
          failWith("Executable files cannot be uploaded.");
          return;
        }
      ...
      failWith("Upload failed unexpectedly.");
    }
  }
  ...
  function handleCancel() {
    if (step === "uploading") {
      failWith("Upload cancellation requested.");
      return;
    }
